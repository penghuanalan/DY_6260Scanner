package cn.chinafst.dy_6260scanner.service;
...
public class BluetoothLeService extends Service {
	...
	private BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
		...
		public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
			if (status == BluetoothGatt.GATT_SUCCESS) {
				LogPrint.e("写入成功" + DecodeUtils.byte2HexStr(characteristic.getValue()));
			}
		};
		...
		public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
			if (status == BluetoothGatt.GATT_SUCCESS) {
				LogPrint.e("读取成功" + DecodeUtils.byte2HexStr(characteristic.getValue()));
			}
		}
		...
		public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
			try {
				...
				LogPrint.e("读取数据" +  DecodeUtils.byte2HexStr(byteData) + "");
				byte cmd = byteData[1];
				switch (cmd) {
	...
	public boolean initialize() {
		if (mBluetoothManager == null) {
			...
			if (mBluetoothManager == null) {
			LogPrint.e("bluetoothmanager无法初始化");
				return false;
			}
		}
		mBluetoothAdapter = mBluetoothManager.getAdapter();
		if (mBluetoothAdapter == null) {
			LogPrint.e("无法获得 ：mBluetoothAdapter");
			return false;
		}
		return true;
	}
